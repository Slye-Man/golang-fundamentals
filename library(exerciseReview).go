...
type BookEntry struct {
	...
	lended int //total books leneded out
}
...
func main() {
	...
	library.books["Learning GO fundamentals"] = BookEntry {
		total: 1,
		lended: 0,
	}

	library.members["Jackson"] = Member{"Jackson", make(map[Title]LendAudit)}
	library.members["John"] = Member{"John", make(map[Title]LendAudit)}
	library.members["Patrick"] = Member{"Patrick", make(map[Title]LendAudit)}

	fmt.Println("\nInitial:")
	printLibraryBooks(&library)
	printMemberAudits(&library)

	member := library.members["Jackson"]
