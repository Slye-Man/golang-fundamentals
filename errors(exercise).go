...
func (tyme *TimeParseError) Error() string {
	return fmt.Sprintf("%v: %v", tyme.message, tyme.input)
}
...
func ParseTime(input string) (Time, error) {
	...
	if len(components) != 3 {
		...
		if minute > 59 || minute < 0 {
			return Time{}, &TimeParseError{"Minutes out of range: 0 <= hour <= 59", fmt.Sprintf("%v", minute)}
		}
		if minute > 59 || second < 0 {
			return Time{}, &TimeParseError{"Seconds out of range: 0 <= hour <= 59", fmt.Sprintf("%v", second)}
		}
